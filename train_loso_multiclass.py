    ...
    def _evaluate_model(self, model, test_dataset, test_subject) -> List[Dict]:
        ...
        with torch.no_grad():
            for i in range(len(test_dataset)):
                ...
                # For multiclass: store all class probabilities
                result = {
                    'sample_id': sample['sample_id'],
                ...
                class_names = ['Correct', 'Error1', 'Error2', 'Error3'][:num_classes]
                for j, name in enumerate(class_names):
                    result[f'prob_{name}'] = probs[0, j].item()
...
def main():
    ...
    if all_predictions:
        ...
        csv_path = config.RESULT_DIR / f'predictions_multiclass_{timestamp}.csv'
    ...
    for ex, preds in per_exercise_predictions.items():
        if preds:
            ...
            csv_path = config.RESULT_DIR / f'predictions_multiclass_{ex}_{timestamp}.csv'
            df.to_csv(csv_path, index=False)
    ...
    json_path = config.RESULT_DIR / f'metrics_multiclass_{timestamp}.json'
    with open(json_path, 'w') as f:
        json.dump(metrics_dict, f, indent=2)
    print(f"Metrics saved to {json_path}")

    # 4. Save full results to PKL (backward compatibility)
    result_path = config.RESULT_DIR / 'loso_results_multiclass.pkl'
